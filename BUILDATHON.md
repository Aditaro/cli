...
- **Multi-table migrations with real parallelism.** Today's slice is deliberately one table. The natural next step is a dependency-aware plan across many tables at once — independent tables migrating concurrently under a declared worker bound, while migrations touching the same table stay strictly ordered, and a failure isolates to its own table instead of stopping the run. (A prototype of exactly this bounded scheduler exists on the `handoff` branch; it is excluded from this submission because it was not live-verified before the deadline.)
...
- **Live progress instead of a single pass/fail at the end.** A long backfill should stream state — rows processed, replay lag, current phase, cost against a warehouse budget — so an operator watches a migration progress rather than waiting blind. That is also what makes health alerts meaningful: stalled progress, lag past an SLO, or a rollback window approaching its VACUUM retention limit are all conditions you want to hear about mid-flight, not afterwards.
- **A human in the loop, mid-migration.** Right now Warden decides alone: validate, then heal or proceed. A real operator wants to intervene between phases — inspect what the validation actually caught, supply or correct data for the offending rows, approve a cutover, or hold one back. The checkpoint boundary already built here is what makes that safe to expose: an operator (or agent) can be shown *whether* context is complete enough to decide, via `resume_report.py`, without the raw checkpoint text being handed around.
- **Harder schema changes than adding a constraint.** Changing a primary key, splitting one table into several, or merging several into one are the migrations that genuinely frighten people — they rewrite relationships rather than columns, so a blind revert is least acceptable exactly where it is most likely to be needed. These need the compatibility-view and write-catch-up machinery below, plus per-shape validation, and they are the strongest argument for the checkpoint-driven explanation: a failed key change is precisely the case where "what was this trying to achieve?" is expensive to reconstruct by hand.
