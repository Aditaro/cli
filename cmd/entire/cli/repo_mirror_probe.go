...
// stays on the client's configured base URL and lets that transport handle a
// 421 to the home core on every tick. That also keeps the wait recoverable for
// its whole duration: the home core answers a foreign-region login JWT with a
// bare 401, and the transport only re-exchanges on one it reached by following
// a 421 (see retryBudget.afterRedirect). Short-circuiting to the home core
// would work until the exchanged token left the cache (cachedTokenTTL, 4m) and
// then fail unrecoverably — strictly worse than not optimising at all.
